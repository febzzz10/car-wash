...
BEGIN
  SELECT RAISE(ABORT, 'vehicle organization scope mismatch') WHERE NOT EXISTS (
...
BEGIN
  SELECT RAISE(ABORT, 'wash job organization or ownership scope mismatch') WHERE NOT EXISTS (
...
BEGIN
  SELECT RAISE(ABORT, 'timer already started')
  WHERE NEW.event_type = 'START' AND EXISTS (
    SELECT 1 FROM timer_events WHERE wash_job_id = NEW.wash_job_id
  );
  SELECT RAISE(ABORT, 'timer is not running')
  WHERE NEW.event_type = 'PAUSE' AND COALESCE((
  ...
  SELECT RAISE(ABORT, 'timer is not paused')
  WHERE NEW.event_type = 'RESUME' AND COALESCE((
    SELECT event_type FROM timer_events
    WHERE wash_job_id = NEW.wash_job_id
    ORDER BY event_at DESC, created_at DESC, rowid DESC LIMIT 1
  ), '') <> 'PAUSE';
  SELECT RAISE(ABORT, 'timer cannot end')
  WHERE NEW.event_type = 'END' AND COALESCE((
    SELECT event_type FROM timer_events
    WHERE wash_job_id = NEW.wash_job_id
    ORDER BY event_at DESC, created_at DESC, rowid DESC LIMIT 1
  ), '') NOT IN ('START', 'PAUSE', 'RESUME');
  SELECT RAISE(ABORT, 'timer events must use chronological server timestamps') WHERE EXISTS (
...
BEGIN
  SELECT RAISE(ABORT, 'vehicle photo asset must be ready and private') WHERE NOT EXISTS (
...
BEGIN
  SELECT RAISE(ABORT, 'sensitive values are forbidden in audit logs') WHERE lower(
