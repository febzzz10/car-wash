...
beforeEach(async () => {
  ...
  await env.DB.batch([
    ...
    env.DB.prepare(
      "INSERT OR IGNORE INTO vehicle_types (id, organization_id, code, name, created_at, updated_at) VALUES ('type-invoice', 'org-invoice', 'SUV', 'SUV', ?, ?)",
    ).bind(timestamp, timestamp),
    env.DB.prepare(
...
describe("immutable invoices", () => {
  it("generates one PDF snapshot, replays safely, and exposes honest WhatsApp fallbacks", async () => {
    ...
    expect(shareBody.data.message).toContain(shareBody.data.secureLink);
