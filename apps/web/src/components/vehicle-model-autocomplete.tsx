...
interface VehicleModelSuggestion {
...
export default function VehicleModelAutocomplete({
  ...
  const [suggestions, setSuggestions] = useState<readonly VehicleModelSuggestion[]>([]);
  ...
  const fetchSuggestions = useCallback(
    async (query: string, seq: number) => {
      ...
      setLoading(true);
      try {
        const result = await api<readonly VehicleModelSuggestion[]>(
          `/vehicle-models${queryString({ q: query, limit: String(MAX_SUGGESTIONS) })}`,
  ...
  function selectSuggestion(suggestion: VehicleModelSuggestion) {
  ...
  function handleSuggestionMouseDown(event: React.MouseEvent, suggestion: VehicleModelSuggestion) {
    event.preventDefault();
